...
public class ClusterDivider {
    ...
    public List<Set<Integer>> dividirEnXGrupos(List<Arista> mst, int cantidadUsuarios, int cantidadGrupos) {
        if (mst == null || mst.isEmpty()) 
            throw new IllegalArgumentException("El MST no puede estar vacío");
        if (cantidadGrupos < 1)
        	throw new IllegalArgumentException("La cantidad de grupos debe 1 o mas");
        Arista maxEdge;
        List<Arista> reducido = new ArrayList<>(mst);
        
        for (int i = 0; i < cantidadGrupos-1; i++) {
        maxEdge = getMaxWeightEdge(reducido);
        reducido.remove(maxEdge);
        }
